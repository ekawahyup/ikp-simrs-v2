...
import { getAllPegawai, addPegawaiRow, deletePegawaiRow } from '@/lib/googleSheets';
...
export async function POST(req: Request) {
  try {
    ...
    const success = await addPegawaiRow(body);
    if (success) {
...
export async function DELETE(req: Request) {
  try {
    ...
    const success = await deletePegawaiRow(index);
    if (success) {
